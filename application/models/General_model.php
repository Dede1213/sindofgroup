...
class General_model extends CI_Model
{
    ...
    function getwhere($table, $parameter, $isresult = false, $limit = false, $group = false, $order = false,$in_param = false,$in = false)
    {
        if ($isresult == 1) {
            if ($limit) {
                $this->db->limit($limit);
    ...
    public  function get_query_natural($query = false,$isresult = false)
    {
